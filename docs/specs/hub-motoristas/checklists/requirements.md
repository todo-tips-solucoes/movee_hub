...
- [x] CHK006 - Estão os requisitos de comportamento do `DELETE /motoristas/:id/vinculo` quando o Entregador NÃO tem vínculo definidos (idempotência do desvínculo vs. 404/409)? [Cobertura, Gap] {auto} — **Resolvido na tarefa 6.2.1 (FASE 6)**: `contracts/motoristas-api.md §DELETE vinculo` especifica explicitamente `204` idempotente (no-op, sem erro) quando o Entregador já está sem vínculo — decisão default por consistência REST (RFC 7231 §4.2.2), registrada como Decisão auditável durante a execução da FASE 6. Implementado em `routes/hub-motoristas.js#DELETE /:id/vinculo`: nenhum UPDATE nem entrada de auditoria são gerados no caminho no-op.
...
- **Gaps abertos que viram tarefa no `create-tasks`**: CHK006 (semântica do DELETE idempotente — **fechado na tarefa 6.2.1, FASE 6**), CHK012 (quantificar ou aceitar SC-002 como qualitativo).
- Resultado geral: feature bem especificada (17 FR + 8 SC + 12 cenários + 12 Decisions + contratos + data-model). Gates `doc-quality` e `owasp-security` já passaram no plan. 3 itens humanos + 2 gaps menores — nenhum bloqueia a decomposição em tasks; CHK006 (fechado) e CHK033 viraram tarefas explícitas.
