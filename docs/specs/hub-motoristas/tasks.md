...
- [x] 8.2.4 Revisitar achado emergente da tarefa 4.1 (trigger `trg_entregador_protege_nome`, migration 0019, bloqueia um 2º `PATCH` de nome mesmo do próprio operador — não só reimportação): decidir com o operador se reedição deve ser permitida e, se sim, desenhar o mecanismo (nova migration expand-only) <!-- onda-012: achado RECONFIRMADO empiricamente (Cenario 4 do E2E); e decisao de produto, nao derivavel da spec com confianca -- ESCALADO para bloqueio humano block-004 (dec-048), aguardando resposta do operador via /feature-00c-resume --> <!-- onda-013: operador RESPONDEU (block-004/dec-048) -- "REEDITAR SEMPRE": reedicao manual sem limite; trigger deve proteger SO a sobrescrita vinda da reimportacao S4. Implementado: migration 0025 (hub_jwt_origem_importacao() + trigger condicionado a essa claim, mesmo mecanismo de hub_boot_recovery/0018) + claim origemImportacao:true emitida SO por upsertEntregadoresDoLote (lib/hub-import-processor.js) via lib/hub-postgrest-jwt.js; PATCH manual (routes/hub-motoristas.js) nunca emite a claim -> sempre passa. Testado de verdade: 6 unit novos (hub-postgrest-jwt-unit.test.js: claim aparece/nao aparece; hub-import-processor.test.js: upsert de Entregador carrega a claim, insert de fatos NAO) + 8 asserts novos em infra/hub/testes/hub-motoristas-integration.sh rodado contra stack efemera real (hub-test-*, nao hub-homolog) -- (a) 2a/3a edicao manual -> 200 PASS; (b) reimport com claim sobre Entregador editado -> nome preservado PASS; (c) reimport com claim sobre Entregador nunca editado -> nome atualizado PASS. `npm test` 372/380 verdes (8 falhas pre-existentes em motorista-integration.test.js, confirmadas via git stash ANTES desta mudanca -- nao relacionadas, DB legado compartilhado). Migration 0025 ainda NAO aplicada no hub_homolog_db (aplicacao + push/PR pendem de autorizacao do operador, CLAUDE.md Governanca) -->
