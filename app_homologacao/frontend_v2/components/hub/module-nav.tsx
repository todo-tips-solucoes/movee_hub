...
const ITEM_CLASSNAME = (active: boolean) =>
  cn(
...
function ItemContent({ modulo, active }: { modulo: HubModulo; active: boolean }) {
  ...
  return (
    <>
      ...
      {active && <span className="ml-auto h-5 w-1 rounded-full bg-sidebar-primary" />}
    </>
  );
}

/** Item de navegação para a sidebar fixa (>= lg) — Link direto, sem Sheet. */
function ItemLink({ modulo, active }: { modulo: HubModulo; active: boolean }) {
  return (
    <Link
