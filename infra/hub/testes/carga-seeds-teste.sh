...
get_var() { awk -F= -v k="$1" '$0 !~ /^[[:space:]]*#/ && $1 == k { sub(/^[^=]*=/, ""); print; exit }' "$ENV_FILE"; }
DB_USER="$(get_var HUB_DB_USER)"; DB_NAME="$(get_var HUB_DB_NAME)"
...
load_dataset() { # load_dataset <nome> <dir>
  local name="$1" dir="$2" header ddl total=0 rows
  ...
  for f in "$dir"/*.csv; do
    psql_t -c "\\copy staging_$name FROM STDIN WITH (FORMAT csv, DELIMITER ';', HEADER true)" <"$f" >/dev/null
  done
