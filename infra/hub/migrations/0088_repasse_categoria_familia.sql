-- 0088 — F1: a regra de família passa a valer também no REPASSE semanal.
--
-- A 0087 ensinou o cálculo da PRODUÇÃO do adiantamento a aceitar, além do nome
-- exato, um token de família ('familia:promocao'), para que a campanha da
-- semana seguinte não ficasse de fora em silêncio. O repasse continuou
-- comparando por nome exato — dois campos parecidos com comportamentos
-- diferentes, armadilha para quem configura (decisão do operador 2026-09-22:
-- estender ao repasse).
--
-- TRÊS funções, cada uma na versão MAIS RECENTE — e não são as da 0067:
--   hub_adiantamento_repasse           -> 0083 (DROP+CREATE, não CREATE OR REPLACE)
--   hub_adiantamento_repasse_fechar    -> 0082
--   hub_adiantamento_repasse_motorista -> 0083
-- ⚠️ `hub_adiantamento_repasse` é redefinida na 0083 com DROP+CREATE, então
-- procurar só por "CREATE OR REPLACE" aponta a 0067 e faz perder as correções
-- daquela migration. Corpos extraídos por script das versões acima; a ÚNICA
-- diferença em cada um é a comparação de categoria.
--
-- `hub_adiantamento_repasse_congelado` e `..._ultimo_fechado` (0086) NÃO
-- entram: leem valores já congelados em ApuracaoRepasseItem, não recalculam.
--
-- COMPATÍVEL: `categoria_casa` aceita exatamente o que o `= ANY` aceitava, e
-- mais os tokens. Nenhuma config tem token no extrato hoje (o campo está NULO
-- em produção), então o resultado não muda até alguém configurar.
--
-- ROLLBACK: reaplicar os corpos originais (0083 para repasse e
-- repasse_motorista; 0082 para repasse_fechar) — ver
-- infra/hub/testes/sql/0088-rollback.sql, gerado e testado junto.
-- ⚠️ Se já houver token de família salvo em `categorias_extrato`, reverter faz
-- o repasse CAIR sem erro: trocar por nomes exatos antes.
...
-- `hub_adiantamento_repasse` foi recriada (DROP+CREATE herdado da 0083), então
-- o privilégio precisa ser concedido de novo — DROP apaga o GRANT.
